...
(function (cash) {
    ...
    // Tabulator
    if (cash("#models-table").length) {
        // Setup Tabulator
        let table = new Tabulator("#models-table", {
            ajaxURL: "/models/get",
            ajaxFiltering: true,
            ajaxSorting: true,
            selectable: 1,
            ...
            ajaxResponse: function(url, params, response){
                cash('#total_models').text(response.total);
                return response;
            },
            columns: [
                {
                        ...
                        return `<div class="flex items-center">
                            ...
                            <div class="ml-4 mr-auto">
                                ...
                                <div class="text-gray-600 text-xs whitespace-nowrap">
                                    ${cell.getData().model_number != null ? cell.getData().model_number : ''}
                                </div>
                            </div>
                        </div>`;
                    },
                    ...
                    formatter(cell, formatterParams) {
                        return `<div>
                            <div class="whitespace-nowrap">${
                                cell.getData().categories != null ? cell.getData().categories.name : ''
                            }</div>
                ...
                },
                {
                    ...
                    formatter(cell, formatterParams) {
                        let a = cash(`<div class="flex lg:justify-center items-center">
                            <a class="btn btn-outline-primary edit flex items-center mr-3" href="/model/update/${cell.getData().id}">
        ...
        function filterHTMLForm() {
            let value = cash("#models-table-filter-value").val();
            let status = cash("#models-table-filter-status").val();
            table.setFilter([
        ...
        cash("#models-table-filter-form")[0].addEventListener(
            "keypress",
            function (event) {
         ...
         cash("#models-table-filter-value").on("change", function (event) {
            if ( cash(this).val().length > 2 ) {
                filterHTMLForm();
            }
        });

        cash('#models-table-filter-status').on('change', function(e){
            filterHTMLForm();
        });

        // On reset filter form
        cash("#models-filter-reset").on("click", function (event) {
            cash("#models-filter-value").val("");
            filterHTMLForm();
        });

        cash('.models-table-collapse').on('click', function(){
            filterHTMLForm();
        });
    }
})(cash);
