...
(function (cash) {
    ...
    // Tabulator
    if (cash("#completed-request-table").length) {
        ...
        // Setup Tabulator
        let table = new Tabulator("#completed-request-table", {
            ajaxURL: "/completed-request",
            ajaxFiltering: true,
            ajaxSorting: true,
            printAsHtml: true,
            printStyled: true,
            ...
            ajaxResponse: function(url, params, response){
                cash('#completed_request_tab_sum').text(response.total);
                return response;
            },
            columns: [
                {
            ...
            rowSelected: function(row){
                ...
                if (cash('#completed-req-tab').hasClass('active')){
                ...
                formFormat.forEach(function(item) {
                    ...
                    if ( item.type == "text") {
                        if ( meaning ){
                            cash('.completed_request .'+item.id).text(meaning);
                        ...
                        if ( meaning ){
                            cash('.completed_request .'+item.id).text(meaning.toLocaleString("de-DE"));
                            cash('.completed_request .'+item.id).closest('.scope').removeClass('hidden');
                        } else {
                            cash('.completed_request .'+item.id).text('');
                            cash('.completed_request .'+item.id).closest('.scope').addClass('hidden');
                        ...
                        if ( meaning ){
                            cash('.completed_request .'+item.id).html(meaning);
                            cash('.completed_request .'+item.id).closest('.accordion-item').removeClass('hidden');
                        } else {
                            cash('.completed_request .'+item.id).html('');
                            cash('.completed_request .'+item.id).closest('.accordion-item').addClass('hidden');
                        }
                        ...
                        cash('.completed_request .'+item.id).html(link);
                        ...
                        cash('.completed_request .'+item.id).html(person);
                    }
        ...
        function filterHTMLForm() {
            let value = cash("#completed-request-table-html-filter-value").val();
            let dept = cash("#completed-request-table-html-department").val();
            let type = cash("#completed-request-table-html-request-tp").val();
            table.setFilter([
        ...
        cash("#completed-request-table-html-filter-form")[0].addEventListener(
            "keypress",
            function (event) {
        ...
        cash("#completed-request-table-html-filter-value").on("change", function (event) {
            if ( cash(this).val().length > 2 ) {
                filterHTMLForm();
            }
        });

        cash('#completed-request-table-html-department').on('change', function(e){
            filterHTMLForm();
        });

        cash('#completed-request-table-html-request-tp').on('change', function(e){
            filterHTMLForm();
        });

        cash('.nav-tabs #completed-req-tab').on("click", function (event) {
            table.refreshFilter();
            resetAccordion();
        });

        function resetAccordion(){
            cash('.completed_request .accordion-item .accordion-btn').each(function(){
