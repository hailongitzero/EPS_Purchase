...
import { format, requestFormat } from "./form-component";
...
(function (cash) {
    ...
    // Tabulator
    if (cash("#all-request-table").length) {
        ...
        // Setup Tabulator
        let table = new Tabulator("#all-request-table", {
            ajaxURL: "/all-request",
            ajaxFiltering: true,
            ajaxSorting: true,
            printAsHtml: true,
            ...
            pagination: "remote",
            paginationSize: 20,
            paginationSizeSelector: [20, 40, 60, 80, true],
            layout: "fitColumns",
            responsiveLayout: "collapse",
            placeholder: "Không có dữ liệu",
            ajaxResponse: function(url, params, response){
                cash('#all_request_tab_sum').text(response.total);
                cash('.total_all_result').text(response.total);
                return response;
            },
            columns: [
                {
                                    ...
                                    <div class="w-10 h-10 image-fit zoom-in mx-auto">
                                        <img alt="${cell.getData().requester != null ? cell.getData().requester['name'] : ''}"
                                            ...
                                            class="tooltip rounded-full" src="${
                                                cell.getData().requester != null && cell.getData().requester['photo'] ? cell.getData().requester['photo'] : 'storage/users/no-user.jpg'
                                            }">
                                    </div>
                                </div>`;
                    },
            ...
            rowSelected: function(row){
                ...
                if (cash('#all-req-tab').hasClass('active')){
                ...
                formFormat.forEach(function(item) {
                    ...
                    if ( item.type == "text") {
                        if ( meaning ){
                            cash('.all_request .'+item.id).text(meaning);
                        ...
                        if ( meaning ){
                            cash('.all_request .'+item.id).html(meaning);
                            cash('.all_request .'+item.id).closest('.accordion-item').removeClass('hidden');
                        } else {
                            cash('.all_request .'+item.id).html('');
                            cash('.all_request .'+item.id).closest('.accordion-item').addClass('hidden');
                        }
                        ...
                        cash('.all_request .'+item.id).html(link);
                    } else if ( item.type == "text_arr" ) {
                        var txt = '';
                        if ( data[field].length ) {
                            data[field].forEach(function(subData, index) {
                                txt += subData[subField] + ' - '
                            });
                            cash('.all_request .'+item.id).closest('.scope').removeClass('hidden');
                        } else {
                            cash('.all_request .'+item.id).closest('.scope').addClass('hidden');
                        }
                        cash('.all_request .'+item.id).html(txt);
                    }
        ...
        function filterHTMLForm() {
            let value = cash("#all-request-table-html-filter-value").val();
            let dept = cash("#all-request-table-html-department").val();
            let type = cash("#all-request-table-html-request-tp").val();
            table.setFilter([
        ...
        cash("#all-request-table-html-filter-form")[0].addEventListener(
            "keypress",
            function (event) {
        ...
        cash("#all-request-table-html-filter-value").on("change", function (event) {
            if ( cash(this).val().length > 2 ) {
                filterHTMLForm();
            }
        });

        cash('#all-request-table-html-department').on('change', function(e){
            filterHTMLForm();
        });

        cash('#all-request-table-html-request-tp').on('change', function(e){
            filterHTMLForm();
        });

        cash('.nav-tabs #all-req-tab').on("click", function (event) {
            table.refreshFilter();
            resetAccordion();
            if (format) {
        ...
        function resetAccordion(){
            cash('.all_request .accordion-item .accordion-btn').each(function(){
