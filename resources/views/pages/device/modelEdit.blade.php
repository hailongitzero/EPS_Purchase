...
<div class="grid grid-cols-12 gap-2 mt-5 box p-5">
    ...
    <div class="col-span-12 lg:col-span-6">
        <div class="box">
            <form action="{{url('model-update')}}" method="POST" autocomplete="off" enctype="multipart/form-data">
                {{ csrf_field() }}
                    ...
                    <div class="flex-1 mt-6 xl:mt-0">
                        ...
                        <div class="col-span-12">
                                <div>
                                    ...
                                    <input id="name" name="name" type="text" class="form-control disabled" placeholder="tên" value="{{ $model->name }}">
                                    <input type="hidden" name="id" value="{{ $model->id }}">
                                </div>
                                <div class="mt-3">
                                    <label for="model_number"class="form-label">Số hiệu</label>
                                    <input id="model_number" name="model_number" type="text" class="form-control disabled" placeholder="số hiệu" value="{{ $model->model_number }}">
                                </div>
                                <div class="mt-3">
                                    ...
                                    <select id="manufacturer" name="manufacturer" data-placeholder="Chọn nhà sản xuất" data-search="true" class="tail-select w-full" required>
                                        ...
                                        <option value="{{ $mft->id }}" {{ $mft->id == $model->manufacturer_id ? "selected" : "" }}>{{ $mft->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mt-3">
                                    ...
                                    <select id="category" name="category" data-placeholder="Chọn Danh mục sản phẩm" data-search="true" class="tail-select w-full" required>
                                        ...
                                        <option value="{{ $cate->id }}" {{ $cate->id == $model->category_id ? "selected" : "" }}>{{ $cate->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mt-3">
                                    ...
                                    <select id="depreciation" name="depreciation" data-placeholder="chọn thời gian khấu hao" class="tail-select w-full" required>
                                        ...
                                        <option value="{{ $depr->id }}" {{ $depr->id == $model->depreciation_id ? "selected" : "" }}>{{ $depr->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="mt-3">
                                    ...
                                    <div class="input-group mt-2 sm:mt-0">
                                        <input id="eol" name="eol" type="number" class="form-control disabled" placeholder="số năm" value="{{ $model->eol }}">
                                        <div class="input-group-text">Tháng</div>
                                    </div>
                                </div>
                                <div class="mt-3">.
                                    <label for="notes"class="form-label">Lưu ý</label>
                                    <textarea id="notes" class="form-control" name="notes" placeholder="Nội dung" >{{ $model->notes }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="w-52 mx-auto xl:mr-0 xl:ml-6">
                        <div class="border-2 border-dashed shadow-sm border-gray-200 dark:border-dark-5 rounded-md p-5">
                            <div class="h-40 relative image-fit cursor-pointer zoom-in mx-auto">
                                <img class="model-img rounded-md" alt="" src="{{$model->image ? url($model->image) : url('storage/placeholders/200x200.jpg') }}">
                            </div>
