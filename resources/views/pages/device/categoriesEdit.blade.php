...
<div class="flex flex-col sm:flex-row items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">Cập nhật Danh mục</h2>
...
<div class="grid grid-cols-12 gap-2 mt-5 box p-5">
    ...
    <div class="col-span-12 lg:col-span-6">
        <div class="box">
            <form action="{{url('category-update')}}" method="POST" autocomplete="off" enctype="multipart/form-data">
                {{ csrf_field() }}
                        ...
                        <div class="grid grid-cols-12 gap-x-5">
                            <div class="col-span-12">
                                <div>
                                    ...
                                    <input id="name" name="name" type="text" class="form-control disabled" placeholder="Input text" value="{{ $category->name }}">
                                ...
                                <div class="mt-3">
                                    ...
                                    <select id="type" name="type" class="tail-select w-full">
                                        ...
                                        <option value="{{$type->category_type}}" {{ $type->category_type == $category->category_type ? 'selected': ''}}>{{$type->category_type}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="w-52 mx-auto xl:mr-0 xl:ml-6">
                        <div class="border-2 border-dashed shadow-sm border-gray-200 dark:border-dark-5 rounded-md p-5">
                            <div class="h-40 relative image-fit cursor-pointer zoom-in mx-auto">
                                <img class="category-img rounded-md" alt="" src="{{ $category->image ? url($category->image) : url('storage/placeholders/200x200.jpg')}}">
