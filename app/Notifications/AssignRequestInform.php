...
class AssignRequestInform extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        $mail = (new MailMessage)
            ->subject('[Đang xử lý] '. $this->data['subject'])
            ->priority(2)
            ->action('Chi tiết', url(config('app.url').'/my-requests?tab=my-req-tab&id='.$this->data['request_id']))
